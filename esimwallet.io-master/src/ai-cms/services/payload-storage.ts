  ...
  return {
    async listPages(options) {
      ...
      if (cluster && !isBlogCluster(cluster)) {
        throw new Error(`AI content listing is only supported for blog/support/about/legal clusters. Use PayloadCMS admin for destinations/products.`);
      }
    ...
    async createPage(data) {
      ...
      if (!isBlogCluster(cluster)) {
        throw new Error(`AI content creation is only supported for blog/support/about/legal clusters. Received: ${cluster}. Destinations and products have complex schemas that require manual creation.`);
      }
      ...
      const localeIndex = pathParts.findIndex(p => ['en', 'es', 'de', 'fr', 'pl', 'ru'].includes(p));
      ...
      const aiMetadata = {
        ...
        aiVisualConfig: data.visualConfig ? (typeof data.visualConfig === 'string' ? data.visualConfig : JSON.stringify(data.visualConfig)) : undefined,
        aiImagePrompts: data.imagePrompts ? (typeof data.imagePrompts === 'string' ? data.imagePrompts : JSON.stringify(data.imagePrompts)) : undefined,
      };
    ...
    async updatePage(id, data) {
      ...
      if (!isBlogCluster(cluster)) {
        throw new Error(`AI content update is only supported for blog/support/about/legal clusters. Use PayloadCMS admin for destinations/products.`);
      }
      ...
      if (data.visualConfig) {
        updateData.aiVisualConfig = typeof data.visualConfig === 'string' 
          ? data.visualConfig 
          : JSON.stringify(data.visualConfig);
      }
      
      if (data.imagePrompts) {
        updateData.aiImagePrompts = typeof data.imagePrompts === 'string'
          ? data.imagePrompts
      ...
      const baseResult = result as { id: string; _status?: string; title?: string };
    ...
    async deletePage(id, options) {
      ...
      if (cluster && !isBlogCluster(cluster)) {
        throw new Error(`AI content deletion is only supported for blog/support/about/legal clusters. Use PayloadCMS admin for destinations/products.`);
      }
    ...
    async getPage(id, options) {
      ...
      if (cluster && !isBlogCluster(cluster)) {
        throw new Error(`AI content retrieval is only supported for blog/support/about/legal clusters. Use PayloadCMS admin for destinations/products.`);
      }
      ...
      try {
        ...
        if (typeof visualConfig === 'string') {
          try { visualConfig = JSON.parse(visualConfig); } catch { /* keep as string */ }
        }
        
        let imagePrompts = baseDoc.aiImagePrompts;
        if (typeof imagePrompts === 'string') {
          try { imagePrompts = JSON.parse(imagePrompts); } catch { /* keep as string */ }
        }
